...
class LowRankDeltaA(nn.Module):
    """LoRA-style 低秩ΔA: 修改S_obj动力学, 秩4+幅度控制防止漂移"""
...
class GatedSSM(nn.Module):
    """v3: 移除delta_bias_proj, 直接接受LowRankDeltaA输出, 增强调制+状态扰动"""
    ...
    def forward(self, x, S, delta_A=None):
        ...
        if delta_A is not None:
            r = torch.clamp(r + delta_A * 0.3, 0, 1)  # 增强: 0.1→0.3
        h_hat = torch.tanh(self.W_h(torch.cat([x, r * S], dim=-1)))
        S_new = (1 - z) * S + z * h_hat
        if delta_A is not None:
            S_new = S_new + delta_A * 0.1  # 新增: 直接状态扰动 (≈ΔA·S)
        return self.output_proj(S_new), S_new


class MetaCognition(nn.Module):
    """v3: LowRankDeltaA替代delta_signal_proj, 独立interp头解梯度绑架"""
    def __init__(self, obj_state_dim=32, meta_state_dim=16, delta_rank=4):
        super().__init__()
        # trace: ΔS_obj(32) + repeat(1) + pred_err/entropy/ext_reward(3) = 36
        ...
        # 独立循环头 — 只用action_repeat, 梯度路径完全独立
        self.loop_head = nn.Sequential(
            nn.Linear(1, 8), nn.ReLU(),
            nn.Linear(8, 1), nn.Sigmoid())
        # 共享头 — 置信度/探索/利用, 用pred_err/entropy/ext_reward
    ...
    def forward(self, delta_S_obj, action_repeat, pred_err, entropy, ext_reward, S_meta):
        ...
        trace = torch.cat([delta_S_obj,
                           ...
                           torch.tensor([pred_err, entropy, ext_reward], device=dev)])
        x = self.trace_encoder(trace)
        _, S_meta_new = self.meta_ssm(x, S_meta)
        delta_A = self.low_rank_delta_a(S_meta_new)
        
        loop = self.loop_head(torch.tensor([action_repeat], device=dev))
        shared = self.interp_shared(torch.tensor([pred_err, entropy, ext_reward], device=dev))
